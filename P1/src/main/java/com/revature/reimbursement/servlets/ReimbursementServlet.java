        ...
        try {
            ...
            if (uris.length == 4 && uris[3].equals("update")) {
                if (role.equals("FINANCE MANAGER")) {
                    ...
                    if (reimbursementUpdateRequest.getAmount() != null) rem.setAmount(reimbursementUpdateRequest.getAmount());
                    if (reimbursementUpdateRequest.getDescription() != null) rem.setDescription(reimbursementUpdateRequest.getDescription());
                    if (reimbursementUpdateRequest.getType() != null) rem.setType_id(reimbursementTypesService.getIdByType(reimbursementUpdateRequest.getType()));

                    reimbursementsService.updateReimbursement(rem);
                }
            }

            else if (uris.length == 4 && uris[3].equals("sort")) {
